      ...
      if (field && data[jsonKey]) {
        if (field.tagName === "INPUT") {
          ...
          for (const option of field.options) {
            if (
              option.text.toLowerCase() === data[jsonKey].toLowerCase()
            ) {
              option.selected = true; // Autofill dropdowns
              break;
            }
          }
        }
      ...
      if (radioButtons.length > 0 && radioValue) {
        for (const button of radioButtons) {
          if (button.value.toLowerCase() === radioValue.toLowerCase()) {
            button.checked = true; // Autofill radio buttons
            break;
          }
        }
      }
